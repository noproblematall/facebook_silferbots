...
if (isset($_GET) && !empty($_GET)) {
  ...
  if (isset($_GET['userpic']) && !empty($_GET['userpic'])) {
    // $index = strripos($_SERVER['REQUEST_URI'], 'userpic=');
    // $userpic = urldecode(substr(($_SERVER['REQUEST_URI']), ((int)$index+8)));
    $userpic = $_GET['userpic'];
  }
  if(isset($_GET['width']) && !empty($_GET['width'])){
    $userpic .= '&width='.$_GET['width'];
  }
  if(isset($_GET['ext']) && !empty($_GET['ext'])){
    $userpic .= '&ext='.$_GET['ext'];
  }
  if(isset($_GET['hash']) && !empty($_GET['hash'])){
    $userpic .= '&hash='.$_GET['hash'];
  }
}
?>
...
<head>
  ...
  fbq('init', '339982839981725');					
  fbq('track', 'PageView');					
  </script>					
  <noscript>					
  <img height="1" width="1" style="display:none"					
  src="https://www.facebook.com/tr?id=339982839981725&ev=PageView&noscript=1"/>					
  </noscript>					
    ...
    <div class="task-wrapper">
      ...
      <div class="container">
            ...
            <table class="table">
              ...
              <tbody>
                ...
                <tr>
                  ...
                  <td><i class="custom_tooltip fa fa-question-circle" aria-hidden="true" title="Lorem ipsum dolor sit amet, consectetur xtres adipiscing elit, sed do eiusmod tempor"></i></td>
              ...
              <div class="custom_card">
                <div class="card_header">
                  <a class="collapsed card-link" data-toggle="collapse" href="#collapseTwo">
                    <i class="fa fa-plus-circle"></i>
                  Ver renta diaria por internación
                  </a>
                </div>
        ...
        <div class="row section4 text-center">
          <div class="col-md-12">
            <input type="button" id="not_now" class="btn custom_button2" value="No voy a contratar ahora">
          </div>
        </div>
        <div class="row section4 text-left mt-3">
          <div class="col-md-12">
            <p class="pl-3 font14">&copy; 2020 Seguro Sociable</p>
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        ...
        <div class="text-center">
          ...
          <input type="button" class="btn custom_button1 mt-3" value="Cerrar" id="modal_close">
        </div>
      </div>
    </div>
  </div>
...
<script src="./jquery_toast/jquery.toast.min.js"></script>
<!-- AdminLTE App -->
<script>
  ...
  $(document).ready(function(){
      ...
      if (user_email !== '' && user_id !== '') {
        ...
        $.ajax({
          ...
          beforeSend: function() {
            ...
            $('#not_now').val('Procesando...')
          },
          success: function(data){
            $('#wait').addClass('display_none')
            $('#not_now').val('No voy a contratar ahora')
            console.log(data)
          }
        })
      }else{
    ...
    if($("input[name='radio1']:checked").val() == "individual"){
      $('#plan5_url').attr('href', 'plan5_indi')
      $('#plan6_url').attr('href', 'plan6_indi')
      $('#plan100_url').attr('href', 'plan100_indi')
    }
    if($("input[name='radio1']:checked").val() == "matrimonial"){
      $('#plan5_url').attr('href', 'plan5_matr')
      $('#plan6_url').attr('href', 'plan6_matr')
      $('#plan100_url').attr('href', 'plan100_matr')
    }
    if($("input[name='radio1']:checked").val() == "familiar"){
      $('#plan5_url').attr('href', 'plan5_fami')
      $('#plan6_url').attr('href', 'plan6_fami')
      $('#plan100_url').attr('href', 'plan100_fami')
    }
    let individual_price = ['$701', '$825', '$1.385']
    let matrimonial_price = ['$1.279', '$1.504', '$2.521']
    let familiar_price = ['$1.900', '$2.234', '$3.746']

    $('#plan5_url').click(function(e){
      e.preventDefault()
      ...
      MessengerExtensions.requestCloseBrowser(function success() {
        ...
        }, function error(err) {console.log('an error occured');}
      );
      
      $.toast({
          heading: 'Bien! Click en "Listo" o "X" ↑ para cerrar esta ventana y continuar',
          text: ' ',
          icon: 'info',
          position: 'top-right',
          ...
          loaderBg: '#FFBD59',  // To change the background
          position: {
              right: 5,
              top: 5
          },
    ...
    $('#not_now').click(function(){
      ...
      fbq('track', 'NoContrataAhora', {		
        content_name: 'No contrata ahora',		
        content_category: 'SA',
        content_type: $("input[name='radio1']:checked").val(),
      });
